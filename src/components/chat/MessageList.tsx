...
import { User, Bot, Loader2, FilePlus, FilePen, Eye, TextCursorInput, Undo2, FileX, FileCog } from "lucide-react";
import { MarkdownRenderer } from "./MarkdownRenderer";

function getToolDisplay(tool: ToolInvocation): { label: string; Icon: React.ElementType } {
  ...
  if (tool.toolName === "str_replace_editor") {
    switch (args.command) {
      case "create":     return { label: `Creating ${filename}`,          Icon: FilePlus };
      case "str_replace":return { label: `Editing ${filename}`,           Icon: FilePen };
      case "view":       return { label: `Reading ${filename}`,           Icon: Eye };
      case "insert":     return { label: `Inserting into ${filename}`,    Icon: TextCursorInput };
      case "undo_edit":  return { label: `Undoing edit in ${filename}`,   Icon: Undo2 };
      default:           return { label: `Working on ${filename}`,        Icon: FileCog };
    }
  }

  if (tool.toolName === "file_manager") {
    switch (args.command) {
      case "rename": return { label: `Renaming ${filename}`, Icon: FilePen };
      case "delete": return { label: `Deleting ${filename}`, Icon: FileX };
      default:       return { label: `Managing ${filename}`, Icon: FileCog };
    }
  }

  return { label: tool.toolName, Icon: FileCog };
}
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation": {
                            ...
                            const { label, Icon } = getToolDisplay(tool);
                            ...
                            return (
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                ...
                                <span className={cn("truncate max-w-[220px]", done ? "text-neutral-600" : "text-neutral-700")}>
                                  {label}
                                </span>
                              </div>
                            );
